...
# Icebreaker bot states
STATE_NONE = 0
STATE_GAME = 1

app = Flask(__name__)
api = Api(app)
BOT_NAME = "IceBreakerBot"
...
# Tracker class to store the conversation state of the icebreaker bot
class IceBreakerState:

    def __init__(self):
        self.state = STATE_NONE

    def get_do_speak(self, rasa_intent, frequency_prob, loudness_prob, distance_prob):
        # TODO: Implement
        
        # Use params to calculate whether Icebreaker Bot should take a turn given the utterance and probability data
        do_speak = True
        
        return do_speak
    
    def on_receive(self, utterance, probs_data):
        
        # Extract probabilities from json object
        # TODO: Check actual keys for this data
        frequency_prob = probs_data['FREQUENCY_PROBABILITY']
        loudness_prob  = probs_data['LOUDNESS_PROBABILITY']
        distance_prob  = probs_data['DISTANCE_PROBABILITY']
        
        # Request RASA for intent using utterance
        intent_data = parse_utterance(utterance)

        # TODO: Extract stuff from intent_data
        rasa_intent = "affirm"
        
        do_speak = get_do_speak(rasa_intent, frequency_prob, loudness_prob, distance_prob)
        
        if do_speak:
            if self.state == STATE_GAME:
                # TODO: Send to games bot
                pass
            else:
                # TODO: Send to other bots
                pass
        else:
            # TODO: Send silence string
            pass
