    ...
    def VisualiseCSV(startTurnSeconds, EndTurnSeconds):
        ...
        for i in range(len(F0Values) - 1, -1, -1):
            if lowerBound > F0Values[i] or F0Values[i] > upperBound:
                marker.append(i)
        for i in range(len(marker) - 3, -1, -1):
            thisI = marker[i]
            thisI1 = marker[i + 1]
            if thisI - thisI1 == 1:
                lastOccurence = thisI1
                break



        F0endOfTurnProb = lastOccurence/LengthOfF0Data*100
        ...
        EnergyendOfTurnProb = lastOccurence / LengthOfEnergyData * 100
        ...
        LoudnessEndOfTurnProb = lastOccurence / LengthOfLoudnessData * 100
